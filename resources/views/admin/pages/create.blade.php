...
@section('admin_content')
    <section class="content-card">
        <div class="section-bar">Add New Post</div>

        @if ($errors->any())
            <div class="flash-error">{{ $errors->first() }}</div>
        @endif

        <form class="form-grid" method="POST" action="{{ route('pages.store') }}" enctype="multipart/form-data">
            @csrf

            <div>
                <label class="field-label" for="meta_title">Meta Title</label>
                <input class="field-input" id="meta_title" name="meta_title" type="text" value="{{ old('meta_title') }}" placeholder="Enter Meta Title">
            </div>

            <div>
                <label class="field-label" for="meta_description">Meta Description</label>
                <textarea class="field-textarea" id="meta_description" name="meta_description" placeholder="Enter Meta Description">{{ old('meta_description') }}</textarea>
            </div>

            <div>
                <label class="field-label" for="page_title">Page Title</label>
                <input class="field-input" id="page_title" name="page_title" type="text" value="{{ old('page_title') }}" placeholder="Enter Keyword Title" required>
            </div>

            <div>
                <label class="field-label" for="image_alt_text">Image Alt Text</label>
                <input class="field-input" id="image_alt_text" name="image_alt_text" type="text" value="{{ old('image_alt_text') }}" placeholder="Enter Image Alt Text">
            </div>

            <div>
                <label class="field-label" for="heading_2">Heading 2</label>
                <input class="field-input" id="heading_2" name="heading_2" type="text" value="{{ old('heading_2') }}" placeholder="Enter Heading 2">
            </div>

            <div>
                <label class="field-label" for="type">Type</label>
                <select class="field-select" id="type" name="type" required>
                    <option value="Post" @selected(old('type') === 'Post')>Post</option>
            ...
            <div>
                ...
                <textarea class="field-textarea js-editor" id="page_description" name="page_description" style="min-height:280px;">{{ old('page_description') }}</textarea>
            </div>
